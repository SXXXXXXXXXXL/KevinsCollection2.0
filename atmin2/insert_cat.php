...
if(isset($_POST['insert_cat'])){
    ...
    if(empty($category_title)){
        $feedback_message = "<div class='bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 p-4 mb-4' role='alert'>Category title cannot be empty.</div>";
    } else {
        ...
        mysqli_stmt_bind_param($stmt_select, "s", $category_title);
        mysqli_stmt_execute($stmt_select);
        $result_select = mysqli_stmt_get_result($stmt_select);
        $number = mysqli_num_rows($result_select);

        if($number > 0){
            $feedback_message = "<div class='bg-red-100 border-l-4 border-red-500 text-red-700 p-4 mb-4' role='alert'>This category is already available.</div>";
        } else {
            ...
            if($result){
                $feedback_message = "<div class='bg-green-100 border-l-4 border-green-500 text-green-700 p-4 mb-4' role='alert'>Category has been added successfully.</div>";
            }
        }
    }
}
...
<div class="max-w-md mx-auto">
    <h2 class="text-2xl font-bold text-center mb-4 text-gray-800">Insert New Category</h2>
    ...
    <form action="" method="post" class="space-y-4">
        ...
        <div>
            <button type="submit" name="insert_cat" 
